...
L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
  maxZoom: 19,
  attribution:
    '&copy; <a href="http://www.openstreetmap.org/copyright">Open Street Map</a>',
}).addTo(map);
...
function applyFilters(feature) {
  return Object.entries(activeFilters).every(([type, filterValue]) => {
    ...
    if (Array.isArray(filterValue)) {
      return filterValue
        .map((v) => v.toUpperCase())
        .includes(String(actualValue).toUpperCase());
    }
...
function renderFilterChips() {
  Object.entries(CHIPS_CONFIG).forEach(([filterKey, listEl]) => {
    ...
    activeFilters[filterKey].forEach((value) => {
      ...
      li.innerHTML =
        li.innerHTML = `<span title="Remove Filter"><button onclick="onFilterChange('${filterKey}', '${value}', true)">✕</button></span> <span>${value}</span>`;
      listEl.append(li);
    });
  });
}
...
stationInput.addEventListener("input", (e) => {
  ...
  displayList(
    ...
    stationData,
    e.target.value,
    (selected) => onFilterChange("owner", selected),
  );
});

stateInput.addEventListener("input", (e) => {
  if (!stateData) return;
  displayList(stateList, stateInput, stateData, e.target.value, (selected) =>
...
cityInput.addEventListener("input", (e) => {
  if (!cityData) return;
  displayList(cityList, cityInput, cityData, e.target.value, (selected) =>
    onFilterChange("city", selected),
  );
});
...
checkBoxes.forEach((checkbox) => {
  checkbox.addEventListener("change", function (e) {
    ...
    if (e.target.checked) {
      console.log(
        `${e.target.nextElementSibling.textContent.trim()} was checked`,
      );
...
function onStateSelect(state) {
  if (state === "--ALL--") {
    cityData = [
      ...new Set(Object.values(geoJsonData.metadata?.cities || {}).flat()),
    ];
    activeFilters.state = [];
  } else {
...
async function loadData() {
  ...
  try {
    ...
    document.getElementById("state-count").textContent =
      geoJsonData.metadata.states.length.toLocaleString();

    document.getElementById("city-count").textContent = Object.values(
      geoJsonData.metadata.cities,
    )
      .flat()
